 ...
 * DESCRIPTION: THIS IS  CODE FOR RUBY-3 TOUCHPANEL(15 switches)
 * touchmatik address: 'k'
 * switches add:A,B,C,D,E,F,G,H,I,J,K,L,M,N,o
...
#define TOTAL_NUMBER_OF_SWITCH (15*2)
...
#define CHAR_TRUE '1'
#define CHAR_FALSE '0'
#define TouchMatikBoardAddress 'k'
/* 
...
unsigned int M13;unsigned int M14;unsigned int M15;
...
void actiontouchPanel(char Switch_Num, char sw_status, char Sw_speed) //
{
    ...
    M1=ON;M2=ON;M3=ON;M4=ON;M5=ON;M6=ON;M7=ON;M8=ON; M9=ON;M10=ON;M11=ON;M12=ON;M13=ON;M14=ON;M15=ON;
 ...
 {  
    ...
    else
    {
        switch(Switch_Num) {
            ...
            case 'K':
            {
                   if(M11 == ON && copy_parentalLockBuffer[11] == CHAR_OFF)
                   {
            ...
            case 'L':
            {
                   if(M12 == ON && copy_parentalLockBuffer[12] == CHAR_OFF)
                   {
                       sendFeedback_TO_Gateway(sw_status,'1','2');
                    //    OUTPUT_RELAY10 = switch_status;  
                        M12 = OFF;
            ...
            case 'M':
            {
                   if(M13 == ON && copy_parentalLockBuffer[13] == CHAR_OFF)
                   {
                       sendFeedback_TO_Gateway(sw_status,'1','3');
                    //    OUTPUT_RELAY10 = switch_status;  
                        M13 = OFF;
           ...
           case 'N':
            {
                   if(M14 == ON && copy_parentalLockBuffer[14] == CHAR_OFF)
                   {
                       sendFeedback_TO_Gateway(sw_status,'1','4');
                    //    OUTPUT_RELAY10 = switch_status;  
                        M14 = OFF;
            ...
            case 'O':
            {
                   if(M15 == ON && copy_parentalLockBuffer[15] == CHAR_OFF)
                   {
                       sendFeedback_TO_Gateway(sw_status,'1','5');
                    //    OUTPUT_RELAY10 = switch_status;  
                        M15 = OFF;
